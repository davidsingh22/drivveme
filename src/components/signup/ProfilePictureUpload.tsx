      ...
      <div className="flex items-center gap-4">
        <div onClick={() => fileInputRef.current?.click()} className="relative cursor-pointer group">
          <Avatar className="h-20 w-20 border-2 border-dashed border-border group-hover:border-primary transition-colors">
            {preview ? <AvatarImage src={preview} alt="Profile preview" /> : <AvatarFallback className="bg-muted"><User className="h-8 w-8 text-muted-foreground" /></AvatarFallback>}
          </Avatar>
        ...
        <div className="flex-1">
          <p className="text-sm text-muted-foreground">{selectedFile ? selectedFile.name : 'Click to upload your profile picture'}</p>
